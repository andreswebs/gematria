...
The remote detection path (http:// / https:// prefix -> remote backend), the remote entry in --wordlist-format, and the GEMATRIA_WORDLIST_TOKEN wiring were removed (gem-ek4u). Auto-selection now covers SQLite, index, companion .idx, and memory only.
